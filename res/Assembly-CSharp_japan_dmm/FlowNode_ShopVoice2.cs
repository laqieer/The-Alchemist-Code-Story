...
public class FlowNode_ShopVoice2 : FlowNodePersistent
{
  ...
  public override void OnActivate(int pinID)
  {
    switch (pinID)
    {
      ...
      case 2003:
        ...
        break;
      case 2004:
        MySound.Voice.StopAll(FlowNode_ShopVoice2.sType.ToString(), this.StopSec);
        break;
      default:
        if (this.mVoice == null)
        {
  ...
  private void Update()
  {
    if (this.mWaitingForPlayEnd)
    {
      ...
      {
        this.mWaitSecAfterPlayEnd -= Time.unscaledDeltaTime;
    ...
    if (FlowNode_ShopVoice2.sDelayCueIDDirect)
      this.mVoice.PlayDirect(FlowNode_ShopVoice2.sDelayCueID);
    else
      this.mVoice.Play(FlowNode_ShopVoice2.sDelayCueID);
    FlowNode_ShopVoice2.sDelayCueID = (string) null;
  }
  ...
  public static string GetEventCueID(FlowNode_ShopVoice2.EType type, DateTime dt, string fileName)
  {
    ...
    foreach (FlowNode_ShopVoice2.DateEvent dateEvent in FlowNode_ShopVoice2.sDateEvent[index])
    {
      if (dateEvent.startDate <= dt && dt <= dateEvent.endDate)
        return dateEvent.cueID;
    }
    return (string) null;
  }
