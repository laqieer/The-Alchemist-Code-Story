...
public static class LocalizedText
{
  ...
  {
    get => LocalizedText.s_UseAssetManager;
    set => LocalizedText.s_UseAssetManager = value;
  }

  public static List<LocalizedText.TextTable> Tables => LocalizedText.mTables;

  public static string LanguageCode
  {
    get => LocalizedText.mLanguageCode;
    set => LocalizedText.mLanguageCode = value;
  }
    ...
    LocalizedText.TextTable overwriteTable = null)
  {
    ...
    else
    {
      ...
      s = !UnityEngine.Object.op_Inequality((UnityEngine.Object) textAsset, (UnityEngine.Object) null) ? (string) null : textAsset.text;
    }
    if (s != null)
    {
  ...
  public static void UnloadAllTables() => LocalizedText.mTables.Clear();
  ...
  {
    LocalizedText.TextTable table = LocalizedText.FindTable(tableID);
    return table != null ? new List<string>((IEnumerable<string>) table.Items.Keys).ToArray() : new string[0];
  }
  ...
  public static string GetResourcePath(string key)
  {
    ...
    return length < 0 ? (string) null : LocalizedText.ComposeTablePath(key.Substring(0, length));
  }
  ...
  public static string ReplaceTag(string text)
  {
    ...
    {
      if (!string.IsNullOrEmpty(coinProductParam.Title))
        text = text.Replace("<selected_product>", coinProductParam.Title);
      if (!string.IsNullOrEmpty(coinProductParam.Description))
        text = text.Replace("<selecter_desc>", coinProductParam.Description);
    }
  ...
  public static string Get(string key, ref bool success)
  {
    ...
    if (table == null)
    {
      table = LocalizedText.InternalLoadTable(tableID);
  ...
  public class TextTable
  {
    ...
    public string FindText(string key) => this.Items.ContainsKey(key) ? this.Items[key] : key;
  }
}
