...
namespace SRPG
{
  ...
  public class FlowNode_ReadMail2 : FlowNode_Network
  {
    ...
    public override void OnActivate(int pinID)
    {
      ...
      {
        case 0:
          MailWindow.MailReadRequestData dataOfClass = DataSource.FindDataOfClass<MailWindow.MailReadRequestData>(((Component) this).gameObject, (MailWindow.MailReadRequestData) null);
          ...
          {
            foreach (MailData mailData in all)
            {
              if (haveCheckScope.CheckReceivable(mailData))
              {
                mailDataList.Add(mailData);
                if (mailData.gifts != null)
                {
                  for (int index = 0; index < mailData.gifts.Length; ++index)
                  {
                    if (mailData.gifts[index] != null)
                      giftDataList.Add(mailData.gifts[index]);
                  }
                }
                haveCheckScope.AddCurrentNum(mailData);
              }
            }
          }
        ...
        case 8:
          ...
          if (pinID == 5)
          {
            if (GlobalVars.SelectUnitTicketDataValue != null)
              str = GlobalVars.SelectUnitTicketDataValue.SelectUnitId;
    ...
    private void SetRewordData(MailData[] mail_datas)
    {
      ...
      for (int index = 0; index < mail_datas.Length; ++index)
      {
        ...
        if (mailData != null)
        {
          ...
          foreach (GiftRecieveItemData giftRecieveItemData in rewardData2.GiftRecieveItemDataDic.Values)
            rewardData1.AddReward(giftRecieveItemData);
    ...
    public override void OnSuccess(WWWResult www)
    {
      if (Network.IsError)
      {
        ...
        GlobalVars.SelectUnitTicketDataValue = (GlobalVars.SelectUnitTicketData) null;
      ...
      else
      {
              ...
              for (int index1 = 0; index1 < mail_datas.Length; ++index1)
              {
                ...
                if (mail_datas[index1].Contains(GiftTypes.ConceptCard))
                {
                  ...
                  for (int index2 = 0; index2 < mail_datas[index1].gifts.Length; ++index2)
                  {
                    ...
                    if (!string.IsNullOrEmpty(conceptCardIname) && mail_datas[index1].gifts[index2].LimitBreakCount > 0)
                      MonoSingleton<GameManager>.Instance.Player.UpdateConceptCardLimitBreakTrophy(conceptCardIname, 0, mail_datas[index1].gifts[index2].LimitBreakCount);
                  }
                }
              }
        ...
        if (GlobalVars.UnitGetReward != null && GlobalVars.UnitGetReward.Params.Count > 0)
          MonoSingleton<GameManager>.Instance.Player.OnUnitGet();
        if (RuneUtility.CountRuneNum(GlobalVars.LastReward.Get()) > 0)
        {
    ...
    private class HaveCheckScope : IDisposable
    {
      ...
      public void AddCurrentNum(MailData mailData)
      {
        ...
        foreach (GiftData gift in mailData.gifts)
        {
          ...
          {
            Dictionary<GiftTypes, int> currentNums;
            (currentNums = this.currentNums)[GiftTypes.ConceptCard] = currentNums[GiftTypes.ConceptCard] + gift.ConceptCardNum;
          }
        }
      }
      ...
      public void Dispose() => this.currentNums.Clear();
    }
  }
}
