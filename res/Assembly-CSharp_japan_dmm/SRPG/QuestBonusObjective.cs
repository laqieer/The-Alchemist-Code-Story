...
namespace SRPG
{
  public class QuestBonusObjective
  {
    ...
    public bool IsProgressMission()
    {
      ...
      {
        DebugUtility.LogError(string.Format("塔ではサポートされていないミッションです。⇒ type : {0}", (object) (int) this.Type));
        return false;
      }
      if (missionTypeAttribute.ProgressJudgeType != QuestMissionProgressJudgeType.NotSupport)
        return true;
      DebugUtility.LogError(string.Format("このミッションはバトルを跨ぐ設定をサポートしていません。⇒ type : {0}", (object) (int) this.Type));
      return false;
    }

    public bool IsMissionTypeAllowLose()
    {
      return !this.IsTakeoverProgress ? this.Type == EMissionType.Killstreak || this.Type == EMissionType.TotalDamagesTakenMin || this.Type == EMissionType.TotalDamagesMin || this.Type == EMissionType.TargetKillstreak || this.Type == EMissionType.BreakObjClashMin || this.Type == EMissionType.TotalGetGemCount_Over : this.IsProgressMission() && this.Type != EMissionType.Finisher;
    }
    ...
    public bool TryGetIntValue(ref int value) => int.TryParse(this.TypeParam, out value);
