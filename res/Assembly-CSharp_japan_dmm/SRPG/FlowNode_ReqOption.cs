...
namespace SRPG
{
  ...
  public class FlowNode_ReqOption : FlowNode_Network
  {
    ...
    public class ApiBase
    {
      ...
      public ApiBase(FlowNode_ReqOption node) => this.m_Node = node;
    ...
    public class Api_OptionSet : FlowNode_ReqOption.ApiBase
    {
      ...
      public override string url => "setoption";
      ...
      public override void Complete(WWWResult www)
      {
        if (Network.IsError)
        {
          if (Network.ErrCode == Network.EErrCode.IllegalComment)
          {
            Network.RemoveAPI();
            Network.ResetError();
            ((Behaviour) this.m_Node).enabled = false;
            UIUtility.SystemMessage(Network.ErrMsg, (UIUtility.DialogResultEvent) (go => this.m_Node.ActivateOutputLinks(220)), systemModal: true);
          }
          else
            this.m_Node.OnRetry();
        }
