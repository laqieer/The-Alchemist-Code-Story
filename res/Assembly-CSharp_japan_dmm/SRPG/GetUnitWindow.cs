...
namespace SRPG
{
  ...
  public class GetUnitWindow : SRPG_FixedList, IFlowInterface, ISortableList
  {
    ...
    private void OnSortModeChange(int index) => this.RefreshData();

    public void RefreshData() => this.ClearItems();
    ...
    public void RefreshPieceUnit(bool clear, UnitSelectListData UnitSelectListData)
    {
      ...
      this.mUnitSelectListData = UnitSelectListData;
    ...
    protected override void OnItemSelect(GameObject go) => this.OnSelectPieceUnit(go);
    ...
    private void OnSelectUnit(GameObject go)
    {
      ...
      else
      {
        ...
        FlowNode_DownloadAssets component = ((Component) this).GetComponent<FlowNode_DownloadAssets>();
    ...
    private void OnSelectPieceUnit(GameObject go)
    {
      ...
      if (dataOfClass == null)
        return;
      GlobalVars.SelectUnitTicketData selectUnitTicketData = new GlobalVars.SelectUnitTicketData();
      if (this.mUnitSelectListData != null && this.mUnitSelectListData.items != null)
      {
        foreach (UnitSelectListItemData selectListItemData in this.mUnitSelectListData.items)
        {
          if (dataOfClass.iname == selectListItemData.iname)
          {
            selectUnitTicketData.SelectUnitId = selectListItemData.iname;
            selectUnitTicketData.ConvertPieceNum = selectListItemData.convert_piece_num;
            GlobalVars.SelectUnitTicketDataValue = selectUnitTicketData;
            GlobalVars.UnlockUnitID = selectListItemData.iname;
            break;
          }
        }
      }
      FlowNode_DownloadAssets component = ((Component) this).GetComponent<FlowNode_DownloadAssets>();
      if (UnityEngine.Object.op_Inequality((UnityEngine.Object) component, (UnityEngine.Object) null) && !string.IsNullOrEmpty(GlobalVars.SelectUnitTicketDataValue.SelectUnitId))
        component.DownloadUnits = new string[1]
        {
          GlobalVars.SelectUnitTicketDataValue.SelectUnitId
        };
