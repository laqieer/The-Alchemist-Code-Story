...
namespace SRPG
{
  public class ConceptCardConditionsParam
  {
    ...
    public bool IsMatchConditions(UnitParam unit_param, JobData job_data)
    {
      return unit_param != null && this.IsMatchElement(unit_param.element) && (this.sex == ESex.Unknown || this.sex == unit_param.sex) && this.IsMatchBirth(unit_param.birthID) && this.IsMatchUnitGroup(unit_param.iname) && this.IsMatchJobGroup(job_data.JobID);
    }

    private bool IsMatchElement(EElement element)
    {
      return this.element_sum <= 0 || this.conditions_elements[element] > 0;
    }

    private bool IsMatchBirth(int unit_birth)
    {
      return this.birth_id == null || this.birth_id.Length <= 0 || Array.IndexOf<int>(this.birth_id, unit_birth) != -1;
    }

    private bool IsMatchUnitGroup(string unit_iname)
    {
      ...
      return this.units_conditions_type == EUseConditionsType.NotMatch ? !flag : flag;
    }

    private bool IsMatchJobGroup(string job_iname)
    {
      ...
      return this.jobs_conditions_type == EUseConditionsType.NotMatch ? !flag : flag;
    }
    ...
    public string GetConditionDescription()
    {
      ...
      if (this.element_sum > 0)
      {
        ...
        for (int index = 0; index < eelementList.Count; ++index)
        {
          if (this.conditions_elements[eelementList[index]] == 1)
          {
            ...
            stringList.Add(LocalizedText.Get("sys.UNIT_ELEMENT_" + (object) (int) eelement));
      ...
      {
        stringList.Add(LocalizedText.Get("sys.SEX_" + (object) (int) this.sex));
        stringList.Add(LocalizedText.Get("sys.CONCEPT_CARD_SKILL_DESCRIPTION_OF"));
      }
