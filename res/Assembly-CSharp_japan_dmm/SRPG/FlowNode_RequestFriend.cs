...
namespace SRPG
{
  ...
  public class FlowNode_RequestFriend : FlowNode_Network
  {
    ...
    public override void OnActivate(int pinID)
    {
      ...
      {
        FriendWindowItem[] componentsInChildren = ((Component) this).GetComponentsInChildren<FriendWindowItem>();
      ...
      if (friendApplyList.Count > 0 || stringList.Count > 0)
      {
        for (int i = 0; i < friendApplyList.Count; ++i)
        {
          ...
          {
            UIUtility.SystemMessage((string) null, LocalizedText.Get("sys.FRIEND_ALREADY_FRIEND"), (UIUtility.DialogResultEvent) (go => this.ActivateOutputLinks(6)));
            return;
          }
          this.apiType |= 2;
        }
    ...
    public override void OnSuccess(WWWResult www)
    {
      ...
      else
      {
        try
        {
          ...
          if (jsonObject.body.blocks != null)
          {
            GlobalVars.BlockList.Clear();
            for (int index = 0; index < jsonObject.body.blocks.Length; ++index)
              GlobalVars.BlockList.Add(jsonObject.body.blocks[index]);
          }
        }
        catch (Exception ex)
        {
        ...
        if (jsonObject.body.errors != null)
        {
          ...
          UIUtility.SystemMessage(stringBuilder.ToString(), (UIUtility.DialogResultEvent) (go => this.Failure()));
        }
        else
          this.Success();
      }
