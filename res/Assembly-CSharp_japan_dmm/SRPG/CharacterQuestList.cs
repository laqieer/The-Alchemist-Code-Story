...
namespace SRPG
{
  ...
  public class CharacterQuestList : SRPG_FixedList, IFlowInterface, ISortableList
  {
    ...
    {
      List<KeyValuePair<QuestParam, bool>> characterQuests = new List<KeyValuePair<QuestParam, bool>>();
      ...
      for (int index = 0; index < condQuests.Count; ++index)
      {
        ...
        if (charaEpisodeList[index] != null && charaEpisodeList[index].IsAvailable && unitData.IsChQuestParentUnlocked(condQuests[index]) || flag)
        {
          ...
          characterQuests.Add(keyValuePair);
        }
        else
        {
    ...
    public static List<KeyValuePair<QuestParam, bool>> GetCollaboSkillQuests(
      ...
      UnitData unitData2)
    {
      List<KeyValuePair<QuestParam, bool>> collaboSkillQuests = new List<KeyValuePair<QuestParam, bool>>();
      List<QuestParam> questList = CollaboSkillQuestList.GetCollaboSkillQuests(unitData1, unitData2);
      QuestParam[] availableQuests = MonoSingleton<GameManager>.Instance.Player.AvailableQuests;
      for (int i = 0; i < questList.Count; ++i)
      {
        bool flag1 = questList[i].IsDateUnlock();
    ...
    private bool OnFilter_Unlock(CharacterQuestData questData)
    {
      if (questData == null)
        return false;
      return questData.Status == CharacterQuestData.EStatus.New || questData.Status == CharacterQuestData.EStatus.Challenged;
    }

    private bool OnFilter_Lock(CharacterQuestData questData)
    {
      return questData != null && questData.Status == CharacterQuestData.EStatus.Lock;
    }

    private bool OnFilter_Complete(CharacterQuestData questData)
    {
      return questData != null && questData.Status == CharacterQuestData.EStatus.Complete;
    }

    private bool OnFilter_ALL(CharacterQuestData questData) => questData != null;
    ...
    private List<CharacterQuestDataChunk> GetCharacterQuestList()
    {
      ...
      QuestParam[] availableQuests = MonoSingleton<GameManager>.Instance.Player.AvailableQuests;
      List<CharacterQuestDataChunk> characterQuestList = new List<CharacterQuestDataChunk>();
      foreach (QuestParam questParam in availableQuests)
      {
        ...
        if (qp.type == QuestTypes.Character)
        {
          ...
          {
            CharacterQuestDataChunk characterQuestDataChunk = characterQuestList.Find((Predicate<CharacterQuestDataChunk>) (cqdc => cqdc.areaName == qp.ChapterID));
            if (characterQuestDataChunk == null)
            {
              characterQuestDataChunk = new CharacterQuestDataChunk();
              characterQuestList.Add(characterQuestDataChunk);
            }
    ...
    private void RefreshFilter()
    {
      ...
      this.SetSortMethod(this.mSortMenu.GetFilters());
    }
