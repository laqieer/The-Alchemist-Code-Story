  ...
  public class ShopBuySetConfirmWindow : MonoBehaviour, IFlowInterface
  {
    ...
    private void Refresh()
    {
      ...
      if (this.mShopitem.IsArtifact)
      {
        ...
        artifactData.GetHomePassiveBuffStatus(ref fixed_status, ref scale_status);
        ...
        {
          ((Selectable) this.m_SetEffectsButton).interactable = MonoSingleton<GameManager>.Instance.MasterParam.ExistSkillAbilityDeriveDataWithArtifact(artifactParam.iname);
      ...
      else
      {
        ...
        if (this.mShopitem.IsSet)
        {
          ...
          {
            GameObject gameObject = index >= this.shop_item_set_list.Count ? UnityEngine.Object.Instantiate<GameObject>(this.ItemTemplate) : ((Component) this.shop_item_set_list[index]).gameObject;
        ...
        DataSource.Bind<ItemParam>(((Component) this).gameObject, MonoSingleton<GameManager>.Instance.GetItemParam(this.mShopitem.iname));
      }
      if (this.mShopitem.IsArtifact)
      {
      ...
      DataSource.Bind<ShopItem>(((Component) this).gameObject, this.mShopitem);
    ...
    public void CloseJobList() => this.mIsShowArtifactJob = false;
    ...
    private void SetSliderValue(float newValue)
    {
      ...
      int num = ShopData.GetBuyPrice(this.mShopitem) * (int) this.AmountSlider.value;
