...
namespace SRPG
{
  public class UnitListSortWindow : FlowWindowBase
  {
    ...
    public override string name => nameof (UnitListSortWindow);
    ...
    private void CacheToggleParam(GameObject toggle_parent_obj)
    {
      ...
      for (int index = 0; index < componentsInChildren.Length; ++index)
      {
        try
        {
          UnitListSortWindow.SelectType key = (UnitListSortWindow.SelectType) Enum.Parse(typeof (UnitListSortWindow.SelectType), ((UnityEngine.Object) componentsInChildren[index]).name);
          ButtonEvent component = ((Component) componentsInChildren[index]).GetComponent<ButtonEvent>();
        ...
        {
          Debug.LogError((object) ("UnitSortWindow トグル名からSelectTypeを取得できない！ > " + ((UnityEngine.Object) componentsInChildren[index]).name + " ( Exception > " + ex.ToString() + " )"));
        }
      }
    }
      ...
      if (!string.IsNullOrEmpty(key1))
      {
        ...
        else
        {
          ...
          if (!string.IsNullOrEmpty(key2))
          {
            ...
            if (PlayerPrefsUtility.HasKey(key2 + "#"))
              this.SetAlignment(UnitListSortWindow.ConvertReverse(PlayerPrefsUtility.GetInt(key2 + "#") != 0));
            this.SaveSelectType();
          }
        }
      }
      ...
      switch (type)
      {
        ...
        default:
          if (type != UnitListSortWindow.SelectType.ATK && type != UnitListSortWindow.SelectType.DEF && type != UnitListSortWindow.SelectType.MAG && type != UnitListSortWindow.SelectType.MND && type != UnitListSortWindow.SelectType.HP && type != UnitListSortWindow.SelectType.SPD && type != UnitListSortWindow.SelectType.COMBINATION)
          {
            ...
            if (type == UnitListSortWindow.SelectType.AWAKE || type == UnitListSortWindow.SelectType.DEX || type == UnitListSortWindow.SelectType.CRI || type == UnitListSortWindow.SelectType.LUK)
    ...
    public static int GetSortStatus(UnitListWindow.Data data, UnitListSortWindow.SelectType type)
    {
      ...
      switch (type)
      {
        case UnitListSortWindow.SelectType.TIME:
          return 0;
        case UnitListSortWindow.SelectType.RARITY:
          return unit != null ? unit.Rarity : 1;
        case UnitListSortWindow.SelectType.LEVEL:
          return unit != null ? unit.Lv : 1;
        case UnitListSortWindow.SelectType.TOTAL:
          return unit == null ? 0 : unit.CalcTotalParameter();
        default:
          ...
          if (type != UnitListSortWindow.SelectType.COMBINATION)
          {
            if (type != UnitListSortWindow.SelectType.JOBRANK)
            {
              if (type != UnitListSortWindow.SelectType.AWAKE)
              {
                ...
                if (type == UnitListSortWindow.SelectType.LUK)
                  return (int) statusParam.luk;
                goto case UnitListSortWindow.SelectType.TIME;
              }
              else
                return unit != null ? unit.AwakeLv : 1;
            }
            else
              return unit != null ? unit.CurrentJob.Rank : 1;
          }
          else
            return unit != null ? unit.GetCombination() : 1;
      }
    }
    ...
    public static UnitListSortWindow.SelectType ConvertSortMode(GameUtility.UnitSortModes oldMode)
    {
      switch (oldMode)
      {
        ...
        case GameUtility.UnitSortModes.Dex:
          return UnitListSortWindow.SelectType.DEX;
        case GameUtility.UnitSortModes.Cri:
          return UnitListSortWindow.SelectType.CRI;
        case GameUtility.UnitSortModes.Luk:
          return UnitListSortWindow.SelectType.LUK;
        default:
      ...
      UnitListSortWindow.SelectType selectType)
    {
      switch (selectType)
      {
        ...
        default:
          ...
          if (selectType == UnitListSortWindow.SelectType.NAME)
            return GameUtility.UnitSortModes.Name;
          if (selectType == UnitListSortWindow.SelectType.DEX)
            return GameUtility.UnitSortModes.Dex;
          if (selectType == UnitListSortWindow.SelectType.CRI)
            return GameUtility.UnitSortModes.Cri;
          return selectType == UnitListSortWindow.SelectType.LUK ? GameUtility.UnitSortModes.Luk : GameUtility.UnitSortModes.Time;
      }
    }
    ...
    public enum SelectType
    {
      ...
      DEX = 16384, // 0x00004000
      CRI = 32768, // 0x00008000
      LUK = 65536, // 0x00010000
    ...
    {
      public override System.Type type => typeof (UnitListSortWindow);
    }
  }
}
