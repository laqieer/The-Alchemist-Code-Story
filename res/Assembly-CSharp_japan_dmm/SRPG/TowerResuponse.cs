...
namespace SRPG
{
  public class TowerResuponse
  {
    ...
    public TowerClear.TOWER_CLEAR_FLAG ClearFlag => (TowerClear.TOWER_CLEAR_FLAG) this.clear;

    public bool IsNotClear => this.ClearFlag == TowerClear.TOWER_CLEAR_FLAG.NOT_CLEAR;

    public bool IsFirstClear
    {
      get => this.ClearFlag == TowerClear.TOWER_CLEAR_FLAG.CLEAR_AND_SCORE && this.arrived_num > 0;
    }

    public bool IsRoundClear
    {
      get => this.ClearFlag == TowerClear.TOWER_CLEAR_FLAG.CLEAR && this.arrived_num == 0;
    }

    public long FreeRecoverTime => this.rtime;

    public DateTime FreeRecoverEndTime
    {
      get
      {
        DateTime freeRecoverEndTime = TimeManager.FromUnixTime(this.rtime);
        TowerParam tower = MonoSingleton<GameManager>.Instance.FindTower(this.TowerID);
        if (tower != null)
          freeRecoverEndTime.AddHours((double) tower.unit_recover_minute);
        return freeRecoverEndTime;
      }
    }
    ...
    public void Deserialize(ReqTowerRank.JSON_TowerRankResponse json)
    {
      ...
      if (json.speed != null)
      {
        ...
        for (int index = 0; index < json.speed.Length; ++index)
        {
          ...
          if (json.speed[index].guild != null)
          {
            this.SpdRank[index].ViewGuild = new ViewGuildData();
            this.SpdRank[index].ViewGuild.Deserialize(json.speed[index].guild);
          }
        }
      }
      if (json.technical != null)
      {
        ...
        for (int index = 0; index < json.technical.Length; ++index)
        {
          ...
          if (json.technical[index].guild != null)
          {
            this.TecRank[index].ViewGuild = new ViewGuildData();
            this.TecRank[index].ViewGuild.Deserialize(json.technical[index].guild);
          }
        }
      }
    ...
    public void Deserialize(JSON_ReqTowerResuponse.Json_TowerStatus json)
    {
      ...
      QuestParam questParam1 = towerFloor.GetQuestParam();
      foreach (QuestParam questParam2 in referenceQuestList)
    ...
    public TowerFloorParam GetCurrentFloor() => this.currentFloor;
    ...
    public int GetUnitDamage(UnitData unit_data)
    {
      ...
      return playerUnit == null ? 0 : playerUnit.dmg;
    }
    ...
    public void CalcEnemyDamage(List<Unit> enemy, bool is_menu = false)
    {
      ...
      enemy.RemoveAll((Predicate<Unit>) (x => !x.IsGimmick && (int) x.CurrentStatus.param.hp <= 0));
    ...
    public int CalcRecoverCost()
    {
      ...
      double num = Math.Ceiling((TimeManager.FromUnixTime(this.rtime).AddMinutes(-1.0) - serverTime).TotalMinutes) / (double) tower.unit_recover_minute;
    ...
    {
      return MonoSingleton<GameManager>.Instance.Player.Units.FindAll((Predicate<UnitData>) (unitData => !unitData.IsRental && unitData.Lv >= 20));
    }

    public TowerResuponse.PlayerUnit FindPlayerUnit(UnitData unit)
    {
      return this.pdeck == null ? (TowerResuponse.PlayerUnit) null : this.pdeck.Find((Predicate<TowerResuponse.PlayerUnit>) (x => x.unitname == unit.UnitParam.iname));
    }

    public bool IsDied_PlayerUnit(UnitData unit)
    {
      if (unit == null)
        return false;
      ...
      return playerUnit != null && playerUnit.isDied;
    }
    ...
    public void OnFloorRanking(ReqTowerFloorRanking.Json_Response json)
    {
      ...
      if (json.speed != null)
      {
        ...
        for (int index = 0; index < json.speed.Length; ++index)
        {
          ...
          if (json.speed[index].guild != null)
          {
            this.FloorSpdRank[index].ViewGuild = new ViewGuildData();
            this.FloorSpdRank[index].ViewGuild.Deserialize(json.speed[index].guild);
          }
        }
      }
      if (json.technical == null)
      ...
      for (int index = 0; index < json.technical.Length; ++index)
      {
        ...
        if (json.technical[index].guild != null)
        {
          this.FloorTecRank[index].ViewGuild = new ViewGuildData();
          this.FloorTecRank[index].ViewGuild.Deserialize(json.technical[index].guild);
        }
      }
    }

    public bool CheckEnemyDeck() => this.edeck != null;
    ...
    public class PlayerUnit
    {
      ...
      public bool isDied => this.is_died == 1;
    }
