...
namespace SRPG
{
  ...
  public class FlowNode_MediaPlayerController : FlowNodePersistent
  {
    ...
    public override void OnActivate(int pinID)
    {
      switch (pinID)
      {
        ...
        case 50:
          ...
          break;
        case 60:
          this.m_MediaPlayerWrapper.Reload();
          break;
      }
    }

    public void Load(string url) => this.m_MediaPlayerWrapper.LoadFromURL(url, false);

    public void SetVolume(float value) => this.m_MediaPlayerWrapper.SetVolume(value);
    ...
    private void OnReady() => this.ActivateOutputLinks(1000);

    private void OnFirstFrameReady() => this.ActivateOutputLinks(1010);

    private void OnFinished() => this.ActivateOutputLinks(1030);

    private void OnBufferingStart() => this.ActivateOutputLinks(1020);

    private void OnBufferingEnd() => this.ActivateOutputLinks(1021);

    private void OnBufferingTimeout() => this.ActivateOutputLinks(1022);

    private void OnError() => this.ActivateOutputLinks(1040);

    private void OnLoadFailed() => this.ActivateOutputLinks(1050);
  }
}
