...
namespace SRPG
{
  ...
  public class RankMatchMissionWindow : SRPG_ListBase, IFlowInterface
  {
    ...
    public void ReceiveReward(VersusRankMissionParam mission)
    {
      ...
      Network.RequestAPI((WebAPI) new ReqRankMatchMissionExec(mission.IName, new Network.ResponseCallback(this.ResponseCallback)));
    }
    ...
    private void Refresh()
    {
      ...
      Network.RequestAPI((WebAPI) new ReqRankMatchMission(new Network.ResponseCallback(this.ResponseCallback)));
    }
      ...
      else
      {
        if (this.mAPIType == RankMatchMissionWindow.APIType.MISSION)
        {
          ...
          versusRankMissionList.Sort((Comparison<VersusRankMissionParam>) ((m1, m2) =>
          {
            ...
            return num1 != num2 ? num2 - num1 : m1.IName.CompareTo(m2.IName);
          }));
          for (int index = 0; index < versusRankMissionList.Count; ++index)
          {
            ...
            {
              RankMatchMissionItem matchMissionItem = UnityEngine.Object.Instantiate<RankMatchMissionItem>(this.ListItem);
              DataSource.Bind<VersusRankMissionParam>(((Component) matchMissionItem).gameObject, data);
              if (mission_progs.ContainsKey(data.IName))
                DataSource.Bind<ReqRankMatchMission.MissionProgress>(((Component) matchMissionItem).gameObject, mission_progs[data.IName]);
              this.AddItem((ListItemEvents) matchMissionItem);
              ((Component) matchMissionItem).transform.SetParent(((Component) this).transform, false);
              ((Component) matchMissionItem).gameObject.SetActive(true);
