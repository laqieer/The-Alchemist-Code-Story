...
namespace SRPG
{
  ...
  public class FlowNode_GenesisSelectedQuestIsCond : FlowNode
  {
    ...
    public override void OnActivate(int pinID)
    {
      ...
      if (quest == null || !quest.IsGenesisStory && !quest.IsGenesisBoss)
      {
      ...
      else
      {
        ...
        {
          if (quests[index] != quest && quests[index].cond_quests != null && (quests[index].IsGenesisStory || quests[index].IsGenesisBoss) && !(quests[index].ChapterID != quest.ChapterID) && (quest.IsGenesisBoss || quests[index].IsGenesisBoss || quests[index].difficulty != quest.difficulty) && !MonoSingleton<GameManager>.Instance.Player.IsQuestAvailable(quests[index].iname) && Array.FindIndex<string>(quests[index].cond_quests, (Predicate<string>) (cond_quest => cond_quest == selected_quest_id)) >= 0)
          {
