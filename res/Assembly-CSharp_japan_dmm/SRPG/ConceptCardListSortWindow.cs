  ...
  public class ConceptCardListSortWindow : MonoBehaviour, IFlowInterface
  {
      ...
      bool is_rarity = false)
    {
      ...
      sortDataList.Sort((Comparison<ConceptCardListSortWindow.SortData>) ((x, y) =>
      {
        if (is_rarity)
        {
          if ((int) x.data.Rarity != (int) y.data.Rarity)
          {
            int num = ((long) (int) x.data.Rarity).CompareTo((long) (int) y.data.Rarity);
            return sort_order_type == ConceptCardListSortWindow.Type.DESCENDING ? num : -num;
          }
          if ((int) x.data.Lv != (int) y.data.Lv)
          {
            int num = ((long) (int) x.data.Lv).CompareTo((long) (int) y.data.Lv);
            return sort_order_type == ConceptCardListSortWindow.Type.DESCENDING ? num : -num;
          }
          if ((int) x.data.AwakeCount != (int) y.data.AwakeCount)
          {
            int num = ((long) (int) x.data.AwakeCount).CompareTo((long) (int) y.data.AwakeCount);
            return sort_order_type == ConceptCardListSortWindow.Type.DESCENDING ? num : -num;
          }
        }
        ...
        return (long) x.data.UniqueID != (long) y.data.UniqueID ? ((long) x.data.UniqueID).CompareTo((long) y.data.UniqueID) : 0;
      }));
      card_list.Clear();
    ...
    private static ConceptCardListSortWindow.Type LoadData()
    {
      ...
      if (string.IsNullOrEmpty(s))
        return ConceptCardListSortWindow.Type.TIME | ConceptCardListSortWindow.Type.ASCENDING;
      int result = 0;
      return !int.TryParse(s, out result) ? ConceptCardListSortWindow.Type.TIME | ConceptCardListSortWindow.Type.ASCENDING : (ConceptCardListSortWindow.Type) result;
    }
    ...
    public static string GetTypeString(ConceptCardListSortWindow.Type type)
    {
      switch (type)
      {
        ...
        default:
          ...
          return type == ConceptCardListSortWindow.Type.AWAKE ? "sys.SORT_AWAKE" : string.Empty;
      }
    }
