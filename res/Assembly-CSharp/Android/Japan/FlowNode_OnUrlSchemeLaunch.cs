...
public class FlowNode_OnUrlSchemeLaunch : FlowNodePersistent
{
  ...
  private void Update()
  {
    ...
    {
      if (!this.StartCheck || !FlowNode_GetCurrentScene.IsAfterLogin() || ((UnityEngine.Object) MonoSingleton<GameManager>.GetInstanceDirect() == (UnityEngine.Object) null || string.IsNullOrEmpty(MonoSingleton<GameManager>.GetInstanceDirect().Player.Name)) || ((UnityEngine.Object) EventSystem.current == (UnityEngine.Object) null || (UnityEngine.Object) EventSystem.current.currentInputModule == (UnityEngine.Object) null || (!EventSystem.current.currentInputModule.enabled || SRPG.Network.IsConnecting)) || (BlockInterrupt.IsBlocked(BlockInterrupt.EType.URL_SCHEME_LAUNCH) || CriticalSection.IsActive || !GlobalVars.IsTutorialEnd))
        return;
      if (GameUtility.GetCurrentScene() == GameUtility.EScene.HOME)
      {
        HomeWindow current = HomeWindow.Current;
        if ((UnityEngine.Object) current == (UnityEngine.Object) null || !current.IsReadyInTown)
          return;
      }
