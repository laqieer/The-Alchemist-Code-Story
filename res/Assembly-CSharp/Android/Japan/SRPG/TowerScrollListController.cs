...
namespace SRPG
{
  ...
  public class TowerScrollListController : MonoBehaviour
  {
      ...
      foreach (RectTransform rectTransform in this.m_ItemList)
      {
        if (rectTransform.gameObject.activeInHierarchy)
        {
          ...
          if (rect.Contains(anchoredPosition))
            this.OnListItemFocus.Invoke(rectTransform.gameObject);
        }
      }
    }
    ...
    private void FocusUpdate()
    {
      ...
      rect.size = this.Cursor.sizeDelta;
      foreach (TowerQuestListItem towerQuestListItem in this.m_ItemList.ConvertAll<TowerQuestListItem>((Converter<RectTransform, TowerQuestListItem>) (item => item.GetComponent<TowerQuestListItem>())))
      {
        if (towerQuestListItem.gameObject.activeInHierarchy)
        {
          Vector2 anchoredPosition = towerQuestListItem.rectTransform.anchoredPosition;
          anchoredPosition.y = this.GetRectTransForm.anchoredPosition.y + anchoredPosition.y;
          towerQuestListItem.OnFocus(rect.Contains(anchoredPosition));
        }
      }
    }
    ...
    public IEnumerator LoadTowerBG(TowerQuestListItem[] tower_quest_list)
    {
      ...
      return (IEnumerator) new TowerScrollListController.\u003CLoadTowerBG\u003Ec__Iterator0() { tower_quest_list = tower_quest_list, \u0024this = this };
    }
