    ...
    public override void OnActivate(int pinID)
    {
      ...
      if (pinID == 11)
      {
        ...
        if (flag)
        {
          ...
          if (num3 >= 0)
            empty = LocalizedText.Get("sys.CONCEPT_CARD_CONFIRMBONUS_CAUTION", new object[1]
            {
          ...
          if (num5 < 0)
            num5 = 0;
          UIUtility.ConfirmBox(empty + LocalizedText.Get("sys.CONCEPT_CARD_CONFIRMBONUSCOUNT_CAUTION", new object[1]
          {
            (object) num5.ToString()
          }), new UIUtility.DialogResultEvent(this.OnConfirmYes), new UIUtility.DialogResultEvent(this.OnConfirmNo), (GameObject) null, false, -1, (string) null, (string) null);
        }
        else
          this.ActivateOutputLinks(1010);
      }
