...
namespace SRPG
{
  ...
  public class QuestArchiveListWindow : MonoBehaviour, IFlowInterface
  {
    ...
    public void Activated(int pinID)
    {
      switch (pinID)
      {
        ...
        case 2:
          ...
          if (!Enum.IsDefined(typeof (QuestArchiveListWindow.ETabType), (object) num))
            break;
          this.ChangeTab((QuestArchiveListWindow.ETabType) num);
          break;
        case 10:
    ...
    private void Awake()
    {
      if ((UnityEngine.Object) this.QuestArchiveTemplate != (UnityEngine.Object) null)
      ...
      foreach (ArchiveParam archive in MonoSingleton<GameManager>.Instance.Archives)
      {
        if (archive.IsAvailable())
        {
          if (num++ == 0 && archive.keys != null && archive.keys.Count > 0)
          {
            ...
            DataSource.Bind<ItemParam>(this.KeyItemCount.gameObject, itemParam, false);
            DataSource.Bind<ItemData>(this.KeyItemCount.gameObject, itemDataByItemParam, false);
            DataSource.Bind<KeyItem>(this.KeyItemCount.gameObject, key, false);
