...
namespace SRPG
{
  public class InspSkillTriggerParam
  {
    ...
    public static bool Deserialize(JSON_InspSkillTriggerParam[] jsons, ref Dictionary<string, InspSkillTriggerParam> insp_skill_trigger_dic)
    {
      ...
      for (int index = 0; index < jsons.Length; ++index)
      {
        ...
        if (!insp_skill_trigger_dic.ContainsKey(json.iname))
        {
          InspSkillTriggerParam skillTriggerParam = new InspSkillTriggerParam();
          if (skillTriggerParam.Deserialize(json))
    ...
    private bool CheckCondition(InspSkillTriggerParam.TriggerData trigger, UnitData unit, List<UnitData> targets, SkillData use)
    {
      switch (trigger.type)
      {
