...
namespace SRPG
{
  ...
  {
    public ReqBtlComEnd(string req_fuid, int opp_rank, int my_rank, BtlResultTypes result, int[] beats, int[] itemSteals, int[] goldSteals, int[] missions, int[] missions_log, string[] fuid, Dictionary<OString, OInt> usedItems, Network.ResponseCallback response, BtlEndTypes apiType, string trophyprog = null, string bingoprog = null)
    {
    ...
    public ReqBtlComEnd(long btlid, int time, BtlResultTypes result, int[] beats, int[] itemSteals, int[] goldSteals, int[] missions, int[] missions_log, string[] fuid, Dictionary<OString, OInt> usedItems, Network.ResponseCallback response, BtlEndTypes apiType, string trophyprog = null, string bingoprog = null, int elem = 0, string rankingQuestEndParam = null, bool is_rehash = false, bool? is_skip = null, BattleCore.Record record = null)
    {
      ...
      this.body = WebAPI.GetRequestString(this.makeBody(false, btlid, time, result, beats, itemSteals, goldSteals, missions, missions_log, fuid, usedItems, response, apiType, trophyprog, bingoprog, elem, rankingQuestEndParam, is_rehash, is_skip, record));
      this.callback = response;
    }
    ...
    private string makeBody(bool is_arena, long btlid, int time, BtlResultTypes result, int[] beats, int[] itemSteals, int[] goldSteals, int[] missions, int[] missions_log, string[] fuid, Dictionary<OString, OInt> usedItems, Network.ResponseCallback response, BtlEndTypes apiType, string trophyprog, string bingoprog, int elem = 0, string rankingQuestEndParam = null, bool is_rehash = false, bool? is_skip = null, BattleCore.Record record = null)
    {
      ...
      if (result == BtlResultTypes.Win && record != null)
      {
        ...
        if (record.mInspSkillUseList.Count != 0)
        {
          ...
          for (int index1 = 0; index1 < record.mInspSkillUseList.Count; ++index1)
          {
            BattleCore.Record.InspSkill mInspSkillUse = record.mInspSkillUseList[index1];
            if (index1 != 0)
              stringBuilder.Append(",");
            stringBuilder.Append("{");
            stringBuilder.Append("\"uiid\":");
            ...
            stringBuilder.Append(",");
            stringBuilder.Append("\"artifact\":[");
            for (int index2 = 0; index2 < mInspSkillUse.mUnitInspList.Count; ++index2)
            {
              Unit.UnitInsp mUnitInsp = mInspSkillUse.mUnitInspList[index2];
              if (index2 != 0)
                stringBuilder.Append(",");
              stringBuilder.Append("{");
              stringBuilder.Append("\"iid\":");
