  ...
  public class RoomPlayerList : MonoBehaviour, IFlowInterface
  {
    ...
    public void Activated(int pinID)
    {
      ...
      else
      {
        if (pinID != 10)
          return;
        DataSource.Bind<JSON_MyPhotonRoomParam>(this.Root, JSON_MyPhotonRoomParam.Parse(PunMonoSingleton<MyPhoton>.Instance.GetCurrentRoom().json), false);
      }
    }
    ...
    private void Awake()
    {
      ...
      DataSource.Bind<JSON_MyPhotonRoomParam>(this.Root, data, false);
      DataSource.Bind<QuestParam>(this.Root, quest, false);
    ...
    private void RefreshItems()
    {
      ...
      for (int index1 = 0; index1 < maxPlayers; ++index1)
      {
        ...
        foreach (MyPhoton.MyPlayer myPlayer in roomPlayerList1)
        {
          if (myPlayer.json != null)
          {
        ...
        DataSource.Bind<JSON_MyPhotonPlayerParam>(uiItem, data2, false);
        DataSource.Bind<JSON_MyPhotonRoomParam>(uiItem, data1, false);
        if (GlobalVars.SelectedMultiPlayRoomType == JSON_MyPhotonRoomParam.EType.TOWER)
        {
          DataSource.Bind<JSON_MyPhotonPlayerParam>(this.PlayerInfoList[index1], data2, false);
          DataSource.Bind<JSON_MyPhotonRoomParam>(this.PlayerInfoList[index1], data1, false);
        ...
        if (data2 != null && componentsInChildren != null)
        {
          for (int index2 = 0; index2 < componentsInChildren.Length; ++index2)
          {
            ...
            if (gameObjectId.ID != null)
              DataSource.Bind<UnitData>(gameObjectId.gameObject, (UnitData) null, false);
          }
          if (data1.draft_type == 1 && !string.IsNullOrEmpty(data2.support_unit))
          {
            ...
            if ((UnityEngine.Object) gameObjectId != (UnityEngine.Object) null && jsonObject != null)
            {
              UnitData data3 = new UnitData();
              data3.Deserialize(jsonObject);
              DataSource.Bind<UnitData>(gameObjectId.gameObject, data3, false);
            }
          }
                ...
                if (GlobalVars.SelectedMultiPlayRoomType == JSON_MyPhotonRoomParam.EType.TOWER)
                {
                  for (int index3 = 0; index3 < componentsInChildren.Length; ++index3)
                  {
                    ...
                    if (gameObjectId.ID != null && (gameObjectId.ID.Equals("unit" + (object) slotId) || flag1) && (!flag1 || index3 == componentsInChildren.Length - 1))
                    {
                      ...
                      {
                        DataSource.Bind<PlayerPartyTypes>(gameObjectId.gameObject, PlayerPartyTypes.MultiTower, false);
                        ...
                        if (flag2)
                        {
                          ...
                          if (unitDataByUnitId != null)
                          {
                            ...
                            DataSource.Bind<UnitData>(gameObjectId.gameObject, unitDataByUnitId, false);
                      ...
                      {
                        DataSource.Bind<UnitData>(this.PlayerInfoList[index1], data3, false);
                        break;
                      }
                      break;
                    }
            ...
            if (myPlayer != null)
            {
              ...
              if (photonPlayerParam != null && photonPlayerParam.units != null && photonPlayerParam.units.Length > 0)
              {
                ...
                if (data2 != null)
                {
                  ...
                  DataSource.Bind<UnitData>(this.SkillObj, data2, false);
    ...
    public void OpenLeaderSkillDetail()
    {
      ...
      if (dataOfClass == null)
        return;
      DataSource.Bind<UnitData>(UnityEngine.Object.Instantiate<GameObject>(this.Prefab_LeaderSkillDetail), dataOfClass, false);
    }
  }
}
