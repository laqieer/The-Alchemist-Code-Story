...
namespace SRPG
{
  ...
  public class VersusDraftPopupList : MonoBehaviour, IFlowInterface
  {
    ...
    private void Awake()
    {
      ...
      if ((UnityEngine.Object) this.mTabItem == (UnityEngine.Object) null)
        return;
      this.mTabItem.gameObject.SetActive(false);
      for (int index = 0; index < versusDraftDecksNow.Count; ++index)
      {
        VersusDraftPopupList.DraftDeck draftDeck = new VersusDraftPopupList.DraftDeck() { mDraftUnitListCache = instance.GetVersusDraftUnits((long) versusDraftDecksNow[index].Id), mUnitList = new List<GameObject>(), mPage = 0 };
        draftDeck.mPageMax = Mathf.CeilToInt((float) draftDeck.mDraftUnitListCache.Count / 28f);
        Toggle toggle = UnityEngine.Object.Instantiate<Toggle>(this.mTabItem, this.mTabItem.transform.parent);
        toggle.onValueChanged.AddListener(new UnityAction<bool>(this.OnChange));
        toggle.gameObject.AddComponent<SerializeValueBehaviour>().list.SetObject("VDPL_TAB_INDEX_KEY", (object) index);
        toggle.gameObject.SetActive(true);
        Text componentInChildren = toggle.transform.GetComponentInChildren<Text>();
    ...
    private void OnChange(bool on)
    {
      if (!on)
        return;
      SerializeValueBehaviour component = this.mTabGroup.ActiveToggles().FirstOrDefault<Toggle>().GetComponent<SerializeValueBehaviour>();
