  ...
  public class ArenaWindow : MonoBehaviour, IFlowInterface, IWebHelp
  {
    ...
    private void BattleStartConfirm()
    {
      if (AssetDownloader.IsEnableShowSizeBeforeDownloading)
        AssetDownloader.StartConfirmDownloadQuestContentYesNo(this.GetBattleEntryUnits(), (List<ItemData>) null, MonoSingleton<GameManager>.Instance.FindQuest(GlobalVars.SelectedQuestID), (UIUtility.DialogResultEvent) (ok => this.DownloadApproved()), (UIUtility.DialogResultEvent) (no => this.DownloadNotApproved()));
      else
    ...
    private void RefreshAttackParty()
    {
      ...
      for (int index = 0; index < this.PartyUnitSlots.Length; ++index)
      {
        ...
        if (index == 0)
        {
          ...
          DataSource.Bind<UnitData>(this.PartyUnitLeaderVS, unitData1, false);
      ...
      {
        DataSource.Bind<PartyData>(this.PartyInfo, partyOfType, false);
        GameParameter.UpdateAll(this.PartyInfo);
      }
      if (!((UnityEngine.Object) this.VsPartyInfo != (UnityEngine.Object) null))
        return;
      DataSource.Bind<PartyData>(this.VsPartyInfo, partyOfType, false);
    ...
    private void RefreshDefenseParty()
    {
      ...
      for (int index = 0; index < this.PartyUnitSlots.Length; ++index)
      {
        ...
        if (index == 0)
        {
          DataSource.Bind<UnitData>(this.DefenseUnitLeader, unitData1, false);
          GameParameter.UpdateAll(this.DefenseUnitLeader);
        }
        DataSource.Bind<UnitData>(this.DefenseUnitSlots[index], unitData1, false);
        GameParameter.UpdateAll(this.DefenseUnitSlots[index]);
      }
      if (!((UnityEngine.Object) this.DefensePartyInfo != (UnityEngine.Object) null))
        return;
      DataSource.Bind<PartyData>(this.DefensePartyInfo, partyOfType, false);
    ...
    private void RefreshEnemyList()
    {
      ...
      for (int index = 0; index < arenaPlayers.Length; ++index)
      {
        ...
        DataSource.Bind<ArenaPlayer>(listItemEvents.gameObject, arenaPlayers[index], false);
    ...
    private void OnEnemySelect(GameObject go)
    {
      ...
      else
      {
        ...
        {
          DataSource.Bind<ArenaPlayer>(this.VsEnemyPartyInfo, dataOfClass, false);
    ...
    private void RefreshBattleCount()
    {
      ...
      for (int index = 0; index < challengeArenaMax; ++index)
        this.BpHolder.transform.Find("bp" + (index + 1).ToString()).gameObject.SetActive(index + 1 <= num);
    }
      ...
      if (arenaPlayer != null && arenaPlayer.Unit != null)
      {
        for (int index = 0; index < arenaPlayer.Unit.Length; ++index)
        {
          if (arenaPlayer.Unit[index] != null)
            unitDataList.Add(arenaPlayer.Unit[index]);
        }
      }
      return unitDataList;
    }
