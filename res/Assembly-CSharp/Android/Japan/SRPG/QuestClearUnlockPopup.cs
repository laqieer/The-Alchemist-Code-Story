  ...
  public class QuestClearUnlockPopup : MonoBehaviour
  {
    ...
    private void Start()
    {
      ...
      for (int index = 0; index < this.mUnlocks.Length; ++index)
      {
        ...
        {
          if (data1 != null)
            DataSource.Bind<AbilityParam>(gameObject, data1, false);
          if (data2 != null)
            DataSource.Bind<SkillParam>(gameObject, data2, false);
    ...
    private void Update()
    {
      ...
      else
      {
        ...
        {
          if (this.CloseButton.gameObject.activeInHierarchy || !GameUtility.CompareAnimatorStateName((Component) this.mWindowAnimator, this.ItemOpendAnimation) || this.mWindowAnimator.IsInTransition(0))
            return;
          this.CloseButton.gameObject.SetActive(true);
        }
        else if (!this.CloseButton.gameObject.activeInHierarchy && this.mListItems.Count <= 1 && (GameUtility.CompareAnimatorStateName((Component) this.mWindowAnimator, this.ItemOpendAnimation) && !this.mWindowAnimator.IsInTransition(0)))
        {
          this.CloseButton.gameObject.SetActive(true);
        }
        else
