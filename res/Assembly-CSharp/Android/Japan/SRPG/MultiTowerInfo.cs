...
namespace SRPG
{
  ...
  public class MultiTowerInfo : MonoBehaviour, IFlowInterface
  {
    ...
    private void Setup(int idx)
    {
      ...
      DataSource.Bind<MultiTowerFloorParam>(this.QuestInfo, mtFloorParam, false);
      DataSource.Bind<QuestParam>(this.QuestInfo, questParam, false);
    ...
    private void FocusUpdate()
    {
      ...
      foreach (MultiTowerFloorInfo multiTowerFloorInfo in this.ItemList.ConvertAll<MultiTowerFloorInfo>((Converter<RectTransform, MultiTowerFloorInfo>) (item => item.GetComponent<MultiTowerFloorInfo>())))
      {
        if (multiTowerFloorInfo.gameObject.activeInHierarchy)
        {
          ...
          multiTowerFloorInfo.OnFocus(rect.Contains(anchoredPosition));
        }
      }
    }
    ...
    public void Activated(int pinID)
    {
      switch (pinID)
      {
        ...
        case 4:
          ...
          if (GlobalVars.SelectedMultiTowerFloor <= 0)
            return;
          if ((bool) ((UnityEngine.Object) this.ScrollList))
