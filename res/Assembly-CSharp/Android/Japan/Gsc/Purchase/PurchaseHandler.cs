...
namespace Gsc.Purchase
{
  public class PurchaseHandler
  {
    ...
    public static void Log(int type, string tag, string message)
    {
      ...
      PurchaseHandler.logger.Post(tag, type != 0 ? LogLevel.Error : LogLevel.Info, message);
    }
    ...
    {
      GameObject gameObject = RootObject.Instance.gameObject;
      if (!((Object) gameObject.GetComponent<PurchaseHandler.Observer>() == (Object) null))
        return;
    ...
    public class Observer : MonoBehaviour
    {
      ...
      {
        if (!focusState || this.cachedInstanceId != this.GetInstanceID())
          return;
        PurchaseFlow.Resume();
      }
    }
