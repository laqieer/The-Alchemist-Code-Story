...
namespace Gsc.Network
{
  public class WebCallbackBuilder<TRequest, TResponse> where TRequest : IRequest<TRequest, TResponse> where TResponse : IResponse<TResponse>
  {
    ...
    public delegate void Callback(WebTask<TRequest, TResponse> callback) where TRequest : IRequest<TRequest, TResponse> where TResponse : IResponse<TResponse>;
    ...
    public class WebCallback : IWebCallback<TRequest, TResponse>
    {
      ...
      {
        return new WebCallbackBuilder<TRequest, TResponse>.WebCallback() { isBehaviour = behaviour != null, behaviour = behaviour, acceptResults = acceptResults, invoke = invoke };
      }
      ...
      public bool OnCallback(WebTask<TRequest, TResponse> task)
      {
        if ((task.Result & this.acceptResults) <= WebTaskResult.None)
          return false;
        if (!this.isBehaviour || (Object) this.behaviour != (Object) null && this.behaviour.enabled)
          this.invoke(task);
        return true;
      }
    }
