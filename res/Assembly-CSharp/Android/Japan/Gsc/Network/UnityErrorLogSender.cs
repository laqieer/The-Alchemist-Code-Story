...
namespace Gsc.Network
{
  public class UnityErrorLogSender : IEnumerator
  {
    ...
    private static Dictionary<string, object> CreateUserData()
    {
      try
      {
        return new Dictionary<string, object>() { { "device_id", (object) Session.DefaultSession.DeviceID }, { "osname", (object) "windows" }, { "platform", (object) Gsc.Auth.Device.Platform } };
      }
      catch (Exception ex)
      {
        return new Dictionary<string, object>() { { "osname", (object) "windows" } };
      }
    }
