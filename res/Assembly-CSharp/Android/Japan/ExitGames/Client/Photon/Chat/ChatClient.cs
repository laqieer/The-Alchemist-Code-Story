...
namespace ExitGames.Client.Photon.Chat
{
  public class ChatClient : IPhotonPeerListener
  {
    ...
    private bool SetOnlineStatus(int status, object message, bool skipMessage)
    {
      ...
      Dictionary<byte, object> customOpParameters = new Dictionary<byte, object>() { { (byte) 10, (object) status } };
      if (skipMessage)
        customOpParameters[(byte) 12] = (object) true;
      else
        customOpParameters[(byte) 3] = message;
      return this.chatPeer.OpCustom((byte) 5, customOpParameters, true);
    }
    ...
    {
      Dictionary<byte, object> customOpParameters = new Dictionary<byte, object>() { { (byte) 0, (object) channels } };
      if (historyLength != 0)
        customOpParameters.Add((byte) 14, (object) historyLength);
      return this.chatPeer.OpCustom(operation, customOpParameters, true);
    }
