  ...
  public static string GetDescription(this TrophyObjective self)
  {
    ...
    switch (self.type)
    {
      ...
      case TrophyConditionTypes.getitem:
        ItemParam itemParam1 = instance.GetItemParam(self.sval_base);
        return string.Format(LocalizedText.Get("sys.TROPHY_GETITEM"), itemParam1 == null ? (object) ("?" + self.sval_base) : (object) itemParam1.name, (object) self.ival);
      ...
      case TrophyConditionTypes.artifacttransmute:
        if (string.IsNullOrEmpty(self.sval_base) || string.IsNullOrEmpty(SRPG_Extensions.GetArtifactName(self.sval_base)))
      ...
      case TrophyConditionTypes.makeartifactlevel:
        return string.Format(LocalizedText.Get("sys.TROPHY_AFENHANCE_ANY"), (object) self.ival);
      ...
      case TrophyConditionTypes.consume:
        List<string> stringList = new List<string>();
        using (List<string>.Enumerator enumerator = self.sval.GetEnumerator())
        {
          while (enumerator.MoveNext())
          {
            string current = enumerator.Current;
            if (current == "COINFREE")
              stringList.Add("Free Gems");
            else if (current == "COINPAID")
              stringList.Add("Paid Gems");
            else if (current == "COINCOM")
              stringList.Add("Community Gems");
            else if (current == "GOLD")
              stringList.Add("Zeni");
            else if (current == "STAMINA")
            {
              stringList.Add("AP");
            }
            else
            {
              ItemParam itemParam2 = MonoSingleton<GameManager>.Instance.GetItemParam(current);
              stringList.Add(itemParam2.name);
            }
          }
        }
        if (self.sval.Count == 1)
          return string.Format(LocalizedText.Get("sys.TROPHY_CONSUME_1"), (object) self.ival, (object) stringList[0]);
        if (self.sval.Count == 2)
          return string.Format(LocalizedText.Get("sys.TROPHY_CONSUME_2"), (object) self.ival, (object) stringList[0], (object) stringList[1]);
        if (self.sval.Count == 3)
          return string.Format(LocalizedText.Get("sys.TROPHY_CONSUME_3"), new object[4]
          {
            (object) self.ival,
            (object) stringList[0],
            (object) stringList[1],
        ...
        if (self.sval.Count == 4)
          return string.Format(LocalizedText.Get("sys.TROPHY_CONSUME_4"), (object) self.ival, (object) stringList[0], (object) stringList[1], (object) stringList[2], (object) stringList[3]);
        if (self.sval.Count == 5)
          return string.Format(LocalizedText.Get("sys.TROPHY_CONSUME_5"), (object) self.ival, (object) stringList[0], (object) stringList[1], (object) stringList[2], (object) stringList[3], (object) stringList[4]);
        return string.Format(LocalizedText.Get("sys.TROPHY_CONSUME_6"), (object) self.ival, (object) stringList[0], (object) stringList[1], (object) stringList[2], (object) stringList[3], (object) stringList[4], (object) stringList[5]);
      case TrophyConditionTypes.job_mastered:
        return string.Format(LocalizedText.Get("sys.TROPHY_JOBMASTER"), (object) MonoSingleton<GameManager>.Instance.GetUnitParam(self.sval[0]).name, (object) MonoSingleton<GameManager>.Instance.GetJobParam(self.sval[1]).name);
      default:
        return string.Empty;
    }
  }
