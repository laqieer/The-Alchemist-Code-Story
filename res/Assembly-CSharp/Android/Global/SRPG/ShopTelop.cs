    ...
    private void UpdateTextString()
    {
      ...
      using (List<ShopTelop.TextChar>.Enumerator enumerator = this.chList.GetEnumerator())
      {
        while (enumerator.MoveNext())
        {
          ...
          string str2 = string.Format("<color=#{0:X2}{1:X2}{2:X2}{3:X2}>", new object[4]{ (object) (byte) ((double) byte.MaxValue * (double) color.r), (object) (byte) ((double) byte.MaxValue * (double) color.g), (object) (byte) ((double) byte.MaxValue * (double) color.b), (object) num });
