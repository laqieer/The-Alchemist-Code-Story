...
namespace SRPG
{
  ...
  public class LimitedShopBuyConfirmWindow : MonoBehaviour, IFlowInterface
  {
    ...
    private void Refresh()
    {
      ...
      LimitedShopItem data1 = MonoSingleton<GameManager>.Instance.Player.GetLimitedShopData().items[GlobalVars.ShopBuyIndex];
      ...
      DataSource.Bind<LimitedShopItem>(this.gameObject, data1);
      if (data1.IsArtifact)
      {
        DataSource.Bind<ArtifactParam>(this.gameObject, MonoSingleton<GameManager>.Instance.MasterParam.GetArtifactParam(data1.iname));
      }
      else
      {
        DataSource.Bind<ItemData>(this.gameObject, MonoSingleton<GameManager>.Instance.Player.FindItemDataByItemID(data1.iname));
