...
namespace SRPG
{
  ...
  public class GachaResultUnitDetail : MonoBehaviour, IFlowInterface
  {
    ...
    private IEnumerator RefreshAsync(bool immediate = false)
    {
      ...
      return (IEnumerator) new GachaResultUnitDetail.\u003CRefreshAsync\u003Ec__IteratorFA() { immediate = immediate, \u003C\u0024\u003Eimmediate = immediate, \u003C\u003Ef__this = this };
    }
    ...
    private void RefreshAbilitList()
    {
      ...
      for (int index = 0; index < learnedAbilities.Count; ++index)
      {
        ...
        if (this.mCurrentUnit.MapEffectAbility != data)
        {
          ...
          GameObject gameObject2 = gameObject1.transform.FindChild("icon").gameObject;
          gameObject1.transform.FindChild("locked").gameObject.SetActive(false);
        ...
        if (learningAbilitys != null && (int) rarityParam.UnitJobLvCap >= lv)
        {
          for (int index = 0; index < learningAbilitys.Length; ++index)
          {
            ...
            if (!string.IsNullOrEmpty(key))
            {
              ...
              gameObject.transform.FindChild("icon").gameObject.GetComponent<ImageArray>().ImageIndex = (int) abilityParam.slot;
    ...
    {
      // ISSUE: object of a compiler-generated type is created
      return (IEnumerator) new GachaResultUnitDetail.\u003CRefreshUnitImage\u003Ec__IteratorFB() { \u003C\u003Ef__this = this };
    }

    private void RefreshGachaCostObject()
    {
      if (GlobalVars.SelectedGachaId == "Rare_Gacha" || GlobalVars.SelectedGachaId == "Rare_Gacha_ii" || GlobalVars.SelectedGachaType == "ticket")
      {
        ...
        this.OneMoreButton.SetActive(false);
    }
  }
}
