...
namespace SRPG
{
  ...
  public class HomeWindow : MonoBehaviour, IFlowInterface
  {
      ...
      if (99 <= pinID && pinID < 128)
      {
        ...
        if (!string.IsNullOrEmpty(desiredSceneName) && (flag2 || this.mLastSceneName != desiredSceneName))
        {
          ...
          if ((MonoSingleton<GameManager>.Instance.Player.TutorialFlags & 1L) == 0L)
          {
            ...
            {
              if (desiredSceneName == "Home_Gacha")
              {
    ...
    private void Start()
    {
      ...
      UnityEngine.Debug.Log((object) "We are setting our flow handler");
      AnalyticsManager.AddPlacementFlow(new Action<string>(this.SetGlobalEventToBeRunCallback));
    ...
    private void OnDestroy()
    {
      ...
      UnityEngine.Debug.Log((object) "We are destroying our flow handler");
      AnalyticsManager.RemovePlacementFlow(new Action<string>(this.SetGlobalEventToBeRunCallback));
    }
    ...
    private void OnApplicationPause(bool pausing)
    {
      ...
      {
        if (trophyprogs1.Count == 1 && trophyprogs1[0].Param.newapi)
          Network.RequestAPIImmediate((WebAPI) new SGReqUpdateTrophy(trophyprogs1, new Network.ResponseCallback(this.OnUpdateTrophyImmediate), false), true);
        else
      ...
      if (trophyprogs2.Count <= 0)
        return;
      if (trophyprogs2.Count == 1 && trophyprogs2[0].Param.newapi)
        Network.RequestAPIImmediate((WebAPI) new SGReqUpdateTrophy(trophyprogs2, new Network.ResponseCallback(this.OnUpdateTrophyImmediate), false), true);
      else
        Network.RequestAPIImmediate((WebAPI) new ReqUpdateTrophy(trophyprogs2, new Network.ResponseCallback(this.OnUpdateTrophyImmediate), false), true);
    }
    ...
    {
      UnityEngine.Debug.Log((object) ("We have set the new pending globalevent " + inPendingGlobalEventName));
      ...
      public override void Update(HomeWindow self)
      {
        ...
        else
        {
          ...
          if (string.IsNullOrEmpty(self.mPendingGlobalEventName) || !self.IsReadyInTown)
            return;
          UnityEngine.Debug.Log((object) ("We are invoking the  " + self.mPendingGlobalEventName + " on " + this.ToString()));
    ...
    private class State_ChangeScene : State<HomeWindow>
    {
      ...
      private IEnumerator ChangeSceneAsync()
      {
        ...
        return (IEnumerator) new HomeWindow.State_ChangeScene.\u003CChangeSceneAsync\u003Ec__Iterator107() { \u003C\u003Ef__this = this };
      }
      ...
      public override void Update(HomeWindow self)
      {
        ...
        if (this.state == 7)
        {
          if (this.asyncOp != null && !this.asyncOp.isDone)
            return;
    ...
    private class State_UpdateTrophy : State<HomeWindow>
    {
      ...
      public override void Begin(HomeWindow self)
      {
        ...
        {
          if (this.mDirtyList.Count == 1 && this.mDirtyList[0].Param.newapi)
            Network.RequestAPI((WebAPI) new SGReqUpdateTrophy(this.mDirtyList, new Network.ResponseCallback(this.ResponseCallback), false), false);
          else
