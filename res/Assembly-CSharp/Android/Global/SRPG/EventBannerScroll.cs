  ...
  public class EventBannerScroll : SRPG_ScrollRect, IFlowInterface
  {
    ...
    private BannerParam[] makeValidBannerParams()
    {
      ...
      for (int index = 0; index < banners.Length; ++index)
      {
        ...
        if (!string.IsNullOrEmpty(banner.banner) && bannerParamList.FindIndex((Predicate<BannerParam>) (p => p.iname == banner.iname)) == -1)
        {
          if (banner.type == BannerType.shop)
          {
            if (instance.IsLimitedShopOpen)
            {
              if (instance.LimitedShopList != null && !string.IsNullOrEmpty(banner.sval))
                Array.Find<JSON_ShopListArray.Shops>(instance.LimitedShopList, (Predicate<JSON_ShopListArray.Shops>) (p => p.gname == banner.sval));
          ...
          else if (banner.type == BannerType.towerQuest && !banner.IsAvailablePeriod(now))
            continue;
    ...
    private IEnumerator movePage(int from, int to)
    {
      ...
      return (IEnumerator) new EventBannerScroll.\u003CmovePage\u003Ec__IteratorF7() { to = to, from = from, \u003C\u0024\u003Eto = to, \u003C\u0024\u003Efrom = from, \u003C\u003Ef__this = this };
    }
