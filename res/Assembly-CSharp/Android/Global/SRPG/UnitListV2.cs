  ...
  public class UnitListV2 : SRPG_FixedList, IFlowInterface, ISortableList
  {
    ...
    private void SGActivated(int pinID)
    {
      ...
      using (List<GameObject>.Enumerator enumerator = this.mItems.GetEnumerator())
      {
        while (enumerator.MoveNext())
        {
          ...
          if (current.GetActive() && current.GetComponent<DataSource>().FindDataOfClass<UnitData>(new UnitData()).UnitID == "UN_V2_LOGI")
          {
            SGHighlightObject.Instance().highlightedObject = current.gameObject;
    ...
    private void RefreshPieceUnit(bool clear)
    {
      ...
      for (int index = 0; index < items.Count; ++index)
      {
        ...
        {
          UnitParam unitParam = Array.Find<UnitParam>(allUnits, (Predicate<UnitParam>) (p => (string) p.piece == item.iname));
