...
namespace SRPG
{
  ...
  public class ChatEditTemplateMessage : MonoBehaviour, IFlowInterface
  {
    ...
    private void InitButtons()
    {
      ...
      {
        this.submit_buttons[index].name = index.ToString();
    ...
    {
      int result = 0;
      if (!int.TryParse(button.name, out result))
        return;
      ...
      else
      {
        ...
        string str = ChatUtility.ReplaceIllegalWordsWithSubstitute(this.input_fields[result].text, this.chat_inspecton_master, '*');
        if (text != str)
        {
