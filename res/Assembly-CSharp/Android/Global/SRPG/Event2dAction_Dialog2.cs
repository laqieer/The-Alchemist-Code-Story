...
namespace SRPG
{
  ...
  public class Event2dAction_Dialog2 : EventAction
  {
    ...
    public override IEnumerator PreloadAssets()
    {
      ...
      return (IEnumerator) new Event2dAction_Dialog2.\u003CPreloadAssets\u003Ec__IteratorB8() { \u003C\u003Ef__this = this };
    }
          ...
          while (enumerator.MoveNext())
          {
            ...
            if (!current.IsClose)
            {
              if (current.CharaID == this.CharaID || this.ContainIgnoreFO(current.CharaID))
              {
                ...
                UnityEngine.Debug.Log((object) ("set index:" + (object) index));
                current.transform.SetSiblingIndex(index);
                current.transform.localScale = Vector3.one * 1f;
    ...
    private void FadeIn(float time)
    {
      ...
      using (List<GameObject>.Enumerator enumerator = this.fadeInList.GetEnumerator())
      {
        while (enumerator.MoveNext())
        {
          ...
          if ((double) current.GetComponent<EventStandChara2>().FaceObject.GetComponent<RawImage>().color.r <= (double) color1.r)
          {
            current.GetComponent<EventStandChara2>().FaceObject.GetComponent<RawImage>().color = color1;
            current.GetComponent<EventStandChara2>().BodyObject.GetComponent<RawImage>().color = color1;
          }
        }
      }
      using (List<GameObject>.Enumerator enumerator = this.fadeOutList.GetEnumerator())
      {
        ...
        {
          GameObject current = enumerator.Current;
          if ((double) current.GetComponent<EventStandChara2>().FaceObject.GetComponent<RawImage>().color.r >= (double) color2.r)
          {
            current.GetComponent<EventStandChara2>().FaceObject.GetComponent<RawImage>().color = color2;
            current.GetComponent<EventStandChara2>().BodyObject.GetComponent<RawImage>().color = color2;
          }
        }
      }
    }
