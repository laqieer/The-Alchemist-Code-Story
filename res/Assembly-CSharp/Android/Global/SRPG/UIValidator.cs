...
namespace SRPG
{
  ...
  public class UIValidator : MonoBehaviour
  {
    ...
    private void Start()
    {
      if ((UnityEngine.Object) this.Input != (UnityEngine.Object) null)
        this.Input.onValueChanged.AddListener(new UnityAction<string>(this.OnInputFieldChange));
      if (this.ExtraInput != null)
      {
        foreach (InputField inputField in this.ExtraInput)
        {
          InputField i = inputField;
          i.onValueChanged.AddListener((UnityAction<string>) (arg0 => this.OnInputFieldChange(arg0, i)));
        }
      }
    ...
    private void UpdateInteractable(CriticalSections csMask)
    {
      ...
      if ((csMask & this.Mask) != (CriticalSections) 0)
        flag = false;
      if ((UnityEngine.Object) this.Input != (UnityEngine.Object) null && string.IsNullOrEmpty(this.Input.text))
        flag = false;
      if (this.ExtraInput != null)
      {
        foreach (InputField inputField in this.ExtraInput)
      ...
      if ((this.ToggleMask & UIValidator.ToggleMasks.BlockRaycast) == (UIValidator.ToggleMasks) 0)
        return;
      CanvasGroup component1 = this.GetComponent<CanvasGroup>();
    ...
    private void OnInputFieldChange(string value)
    {
      for (int startIndex = 0; startIndex < value.Length; ++startIndex)
      {
        ...
        {
          this.Input.text = value.Remove(startIndex);
          break;
        }
      }
      this.UpdateInteractable(CriticalSection.GetActive());
    ...
    private void OnInputFieldChange(string value, InputField input)
    {
      for (int startIndex = 0; startIndex < value.Length; ++startIndex)
      {
        if (this.IsEmoji(value[startIndex]))
        {
          input.text = value.Remove(startIndex);
          break;
        }
      }
      this.UpdateInteractable(CriticalSection.GetActive());
