...
namespace SRPG
{
  public class ScrollListController : MonoBehaviour
  {
    ...
    public void Refresh()
    {
      using (List<ScrollListSetUp>.Enumerator enumerator = ((IEnumerable<MonoBehaviour>) this.GetComponents<MonoBehaviour>()).Where<MonoBehaviour>((Func<MonoBehaviour, bool>) (item => item is ScrollListSetUp)).Select<MonoBehaviour, ScrollListSetUp>((Func<MonoBehaviour, ScrollListSetUp>) (item => item as ScrollListSetUp)).ToList<ScrollListSetUp>().GetEnumerator())
      {
        while (enumerator.MoveNext())
        {
          ScrollListSetUp current = enumerator.Current;
          for (int idx = 0; idx < this.m_ItemList.Count; ++idx)
          {
            ...
            current.OnUpdateItems(idx, rectTransform.gameObject);
          }
        }
      }
      this.m_PrevPosition = 0.0f;
