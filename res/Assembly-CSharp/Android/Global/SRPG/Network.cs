...
namespace SRPG
{
  ...
  public class Network : MonoSingleton<SRPG.Network>
  {
    ...
    private SRPG.Network.MultiLanguage_Message mMultiLanguage_Message;
    ...
    public static SRPG.Network.MultiLanguage_Message MultiMessage
    {
      get
      {
        return MonoSingleton<SRPG.Network>.Instance.mMultiLanguage_Message;
      }
      set
      {
        UnityEngine.Debug.Log((object) "setting iInstance.mMultiLanguage_Message");
        MonoSingleton<SRPG.Network>.Instance.mMultiLanguage_Message = value;
      }
    }

    public static void SetServerMaintainenceMessage(SRPG.Network.MultiLanguage_Message msg)
    {
      SRPG.Network.MultiMessage = msg;
    }
    ...
    public static void ConnectingResponse(WebResponse response, SRPG.Network.ResponseCallback callback)
    {
      ...
      if (!string.IsNullOrEmpty(SRPG.Network.ErrMsg) && SRPG.Network.ErrMsg.Contains("500"))
      {
        ...
        SRPG.Network.ErrMsg = LocalizedText.Get("errorcode." + ((int) SRPG.Network.ErrCode).ToString() + "_MESSAGE");
    ...
    private static IEnumerator Connecting(WebAPI api)
    {
      ...
      return (IEnumerator) new SRPG.Network.\u003CConnecting\u003Ec__Iterator55() { api = api, \u003C\u0024\u003Eapi = api };
    }
    ...
    private static long GetSystemUptime()
    {
      ...
      return SRPG.Network.mSystemClock.CallStatic<long>("elapsedRealtime") / 1000L;
    }
    ...
    public class MultiLanguage_Message
    {
      public MultiLanguage_Message(Document document)
      {
        this.English = document.Root["multi_msg"]["en"].ToString();
        this.French = document.Root["multi_msg"]["fr"].ToString();
        this.German = document.Root["multi_msg"]["de"].ToString();
        this.Spanish = document.Root["multi_msg"]["es"].ToString();
      }
    ...
    public enum EErrCode
    {
      ...
      VS_RankOutOfPeriod = 10019, // 0x00002723
      VS_RankShort = 10020, // 0x00002724
      VS_ReportDuplicate = 10098, // 0x00002772
      VS_ReportNoOwner = 10099, // 0x00002773
