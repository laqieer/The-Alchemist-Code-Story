...
namespace SRPG
{
  public class BattleCore
  {
    ...
    private static EUnitDirection[] leftDirection = new EUnitDirection[5]
    {
    ...
    private static EUnitDirection[] rightDirection = new EUnitDirection[5]
    {
      EUnitDirection.NegativeY,
      EUnitDirection.PositiveX,
      EUnitDirection.PositiveY,
...
label_62:
    ...
    private void DamageSkill(Unit self, Unit target, SkillData skill, LogSkill log)
    {
      ...
      if (!this.IsBattleFlag(EBattleFlag.PredictResult))
      {
        if (!is_avoid)
        {
          ...
          if (this.mQuestParam.type == QuestTypes.RankMatch)
            SceneBattle.Instance.SendBattleDamage((int) self.UnitData.UniqueID, num3);
        }
        else
        {
          num3 = 0;
    ...
    public IEnumerator UnitStartAsync()
    {
      ...
      return (IEnumerator) new BattleCore.\u003CUnitStartAsync\u003Ec__Iterator58() { \u003C\u003Ef__this = this };
    }
    ...
    private bool UpdateMapAI_Impl(bool forceAI)
    {
      ...
      if (action != null && self.IsEnableActionCondition() && (string.IsNullOrEmpty((string) action.skill) && (int) action.type == 0))
      {
        this.CommandWait(false);
      ...
      if (!self.IsUnitFlag(EUnitFlag.Action))
      {
        ...
        else
        {
          ...
          {
            if (action != null && this.CalcUseActionAI(self, action, useskill))
              return true;
...
label_64:
    ...
    private bool CalcMoveTargetAI(Unit self, bool forceAI)
    {
      ...
      for (int index1 = 0; index1 < list.Count; ++index1)
      {
        ...
        Grid[] gridArray = new Grid[4]{ map[unitGridPosition.x - 1, unitGridPosition.y], map[unitGridPosition.x + 1, unitGridPosition.y], map[unitGridPosition.x, unitGridPosition.y + 1], map[unitGridPosition.x, unitGridPosition.y - 1] };
        int index2 = -1;
        for (int index3 = 0; index3 < gridArray.Length; ++index3)
        {
          if (gridArray[index3] != null && gridArray[index3].step != (byte) 127 && Math.Abs(unitGridPosition.height - gridArray[index3].height) <= this.mSkillMap.attackHeight && (index2 == -1 || (int) gridArray[index3].step < (int) gridArray[index2].step))
            index2 = index3;
        }
        if (index2 != -1)
        {
          moveGoalTarget.goal.x = (float) gridArray[index2].x;
          moveGoalTarget.goal.y = (float) gridArray[index2].y;
