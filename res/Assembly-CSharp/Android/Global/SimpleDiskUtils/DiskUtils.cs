...
namespace SimpleDiskUtils
{
  public class DiskUtils
  {
    ...
    {
      return new AndroidJavaClass("com.dikra.diskutils.DiskUtils").CallStatic<int>("availableSpace", new object[1]{ (object) isExternalStorage });
    }

    public static int CheckTotalSpace(bool isExternalStorage = true)
    {
      return new AndroidJavaClass("com.dikra.diskutils.DiskUtils").CallStatic<int>("totalSpace", new object[1]{ (object) isExternalStorage });
    }

    public static int CheckBusySpace(bool isExternalStorage = true)
    {
      return new AndroidJavaClass("com.dikra.diskutils.DiskUtils").CallStatic<int>("busySpace", new object[1]{ (object) isExternalStorage });
    }
    ...
    public static void SaveFile(object obj, string filePath)
    {
      ...
      while (length > 0 && filePath[length - 1] != '/')
        --length;
      if (length <= 0)
