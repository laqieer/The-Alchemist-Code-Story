...
namespace Gsc.Network.Parser
{
  public class Des
  {
    ...
    public static T ToObject<T>(IValue v) where T : class, IResponseObject
    {
      if (v.IsNull())
        return (T) null;
      return AssemblySupport.CreateInstance<T>(new object[1]
      {
        (object) v.GetObject()
      });
    }
