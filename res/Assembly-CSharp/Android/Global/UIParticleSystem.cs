...
public class UIParticleSystem : Graphic
{
  ...
  public void AdvanceTime(float dt)
  {
    ...
    if ((double) this.mSpawnCount >= 1.0)
    {
      ...
      for (int index = 0; index < num && this.mParticles.Count < this.maxParticles; ++index)
      {
        ...
        particle.randomSeed = UnityEngine.Random.Range(0, (int) ushort.MaxValue);
        ...
        switch (this.emitterType)
        {
          case UIParticleSystem.EmitterTypes.Cone:
            ...
            {
              particle.velocity = Quaternion.AngleAxis(UnityEngine.Random.value * 360f, Vector3.forward) * (Vector3) Vector2.up;
              break;
            }
            break;
          case UIParticleSystem.EmitterTypes.Sphere:
            ...
            {
              particle.velocity = Quaternion.AngleAxis(UnityEngine.Random.value * 360f, Vector3.forward) * (Vector3) Vector2.up;
              particle.position = Quaternion.AngleAxis(UnityEngine.Random.value * 360f, Vector3.forward) * (Vector3) Vector2.up * this.sphereEmitter.Radius.Evaluate();
            }
            else
            {
              particle.velocity = Quaternion.AngleAxis(UnityEngine.Random.value * 360f, Vector3.forward) * (Vector3) Vector2.up;
          ...
          case UIParticleSystem.EmitterTypes.Box:
            particle.velocity = !this.boxEmitter.RandomDirection ? Quaternion.AngleAxis(this.emitterRotation, Vector3.forward) * (Vector3) Vector2.up : Quaternion.AngleAxis(UnityEngine.Random.value * 360f, Vector3.forward) * (Vector3) Vector2.up;
            particle.position = new Vector3((UnityEngine.Random.value - 0.5f) * this.boxEmitter.Width, (UnityEngine.Random.value - 0.5f) * this.boxEmitter.Height, 0.0f);
            break;
        }
  ...
  public struct FloatRange
  {
    ...
    {
      return Mathf.Lerp(this.Min, this.Max, UnityEngine.Random.value);
    }
  }
  ...
  public struct ColorRange
  {
    ...
    {
      return Color.Lerp(this.Min, this.Max, UnityEngine.Random.value);
    }
  }
