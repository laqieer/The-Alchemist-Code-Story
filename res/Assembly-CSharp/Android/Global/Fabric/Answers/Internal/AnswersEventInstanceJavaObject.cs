...
namespace Fabric.Answers.Internal
{
  internal class AnswersEventInstanceJavaObject
  {
      ...
      using (Dictionary<string, object>.Enumerator enumerator = customAttributes.GetEnumerator())
      {
        while (enumerator.MoveNext())
        {
          ...
          if (o == null)
            ...
            this.javaObject.Call<AndroidJavaObject>("putCustomAttribute", (object) key, (object) AnswersEventInstanceJavaObject.AsDouble(o));
          else if (o is string)
            this.javaObject.Call<AndroidJavaObject>("putCustomAttribute", (object) key, o);
          else
    ...
    public void InvokeSafelyAsCurrency(string methodName, string currency)
    {
      ...
      {
        (object) new AndroidJavaClass("java.util.Currency").CallStatic<AndroidJavaObject>("getInstance", new object[1]
        {
          (object) currency
        })
      });
