...
namespace SRPG
{
  ...
  public class FlowNode_ReqGuildRaidRankingPortBoss : FlowNode_Network
  {
    ...
    {
      ReqGuildRaidRankingPortBoss.Response response = (ReqGuildRaidRankingPortBoss.Response) null;
      bool flag = EncodingTypes.IsJsonSerializeCompressSelected(this.SerializeCompressMethod);
      if (!flag)
      {
        FlowNode_ReqGuildRaidRankingPortBoss.MP_Response mpResponse = SerializerCompressorHelper.Decode<FlowNode_ReqGuildRaidRankingPortBoss.MP_Response>(www.rawResult, true, EncodingTypes.GetCompressModeFromSerializeCompressMethod(this.SerializeCompressMethod));
      ...
      else
      {
        if (flag)
        {
          WebAPI.JSON_BodyResponse<ReqGuildRaidRankingPortBoss.Response> jsonObject = JSONParser.parseJSONObject<WebAPI.JSON_BodyResponse<ReqGuildRaidRankingPortBoss.Response>>(www.text);
        ...
        try
        {
          GuildRaidManager.Instance.SetupPreviousRankingPortBoss(response.ranking_port_boss, this.Overwrite);
          GuildRaidManager.Instance.RankingPortBossPageTotal = response.totalPage;
        }
        catch (Exception ex)
        {
