  ...
  public class EventAction_MoveCamera2 : EventAction
  {
    ...
    public override void OnActivate()
    {
      ...
      if ((double) this.MoveTime <= 0.0 && this.Speed == EventAction_MoveCamera2.CameraMoveSpeeds.Immediate)
      {
        ...
        actionMoveCamera2.TargetPosition = Vector3.op_Addition(actionMoveCamera2.TargetPosition, Vector3.op_Multiply(Vector3.up, instance.GameCamera_UnitHeightOffset));
        this.UpdateCameraPosition(1f);
        this.ActivateNext();
      }
      else
      {
        TargetCamera targetCamera = TargetCamera.Get((Component) Camera.main);
        targetCamera.Reset();
        ...
        else
        {
          ...
          EventAction_MoveCamera2 actionMoveCamera2 = this;
          actionMoveCamera2.StartTargetPosition = Vector3.op_Addition(actionMoveCamera2.StartTargetPosition, Vector3.op_Multiply(Vector3.up, instance.GameCamera_UnitHeightOffset));
        }
        if (this.SnapToGround)
          this.TargetPosition = GameUtility.RaycastGround(this.TargetPosition);
        EventAction_MoveCamera2 actionMoveCamera2_1 = this;
        actionMoveCamera2_1.TargetPosition = Vector3.op_Addition(actionMoveCamera2_1.TargetPosition, Vector3.op_Multiply(Vector3.up, instance.GameCamera_UnitHeightOffset));
        float num1 = 1f;
        switch (this.Speed)
        {
        ...
        if ((double) this.MoveTime <= 0.0)
        {
          ...
          float num2 = ((Vector3) ref vector3).magnitude;
          if ((double) num2 <= 0.0)
          {
    ...
    public override void Update()
    {
      ...
      if ((this.FadeMode == EventAction_MoveCamera2.CameraFadeModes.FadeIn || this.FadeMode == EventAction_MoveCamera2.CameraFadeModes.FadeInOut) && (double) this.mCurrentTime < (double) this.FadeTime)
        FadeController.Instance.FadeTo(new Color(0.0f, 0.0f, 0.0f, (float) (1.0 - (double) this.mCurrentTime / (double) this.FadeTime)), 0.0f);
      if ((this.FadeMode == EventAction_MoveCamera2.CameraFadeModes.FadeOut || this.FadeMode == EventAction_MoveCamera2.CameraFadeModes.FadeInOut) && (double) this.mCurrentTime >= (double) this.mEndTime - (double) this.FadeTime)
        FadeController.Instance.FadeTo(new Color(0.0f, 0.0f, 0.0f, (this.mCurrentTime - (this.mEndTime - this.FadeTime)) / this.FadeTime), 0.0f);
    ...
    private void UpdateCameraPosition(float t)
    {
      ...
      Camera.main.fieldOfView = instance.GameCamera_TacticsSceneFOV;
    }
