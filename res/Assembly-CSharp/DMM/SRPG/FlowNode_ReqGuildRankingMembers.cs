...
namespace SRPG
{
  ...
  public class FlowNode_ReqGuildRankingMembers : FlowNode_Network
  {
    ...
    public override void OnActivate(int pinID)
    {
      ...
      this.ExecRequest((WebAPI) new ReqGuildRankingMembers(this.m_TargetGid, new SRPG.Network.ResponseCallback(((FlowNode_Network) this).ResponseCallback), this.SerializeCompressMethod));
    }
    ...
    {
      ReqGuildRankingMembers.Response json = (ReqGuildRankingMembers.Response) null;
      bool flag = EncodingTypes.IsJsonSerializeCompressSelected(this.SerializeCompressMethod);
      if (!flag)
      {
        FlowNode_ReqGuildRankingMembers.MP_Response mpResponse = SerializerCompressorHelper.Decode<FlowNode_ReqGuildRankingMembers.MP_Response>(www.rawResult, true, EncodingTypes.GetCompressModeFromSerializeCompressMethod(this.SerializeCompressMethod));
      ...
      else
      {
        if (flag)
        {
          WebAPI.JSON_BodyResponse<ReqGuildRankingMembers.Response> jsonObject = JSONParser.parseJSONObject<WebAPI.JSON_BodyResponse<ReqGuildRankingMembers.Response>>(www.text);
        ...
        try
        {
          if (UnityEngine.Object.op_Inequality((UnityEngine.Object) CombatPowerRanking.Instance, (UnityEngine.Object) null))
            CombatPowerRanking.Instance.SetupGuildMembersRankingData(json);
        }
        catch (Exception ex)
        {
