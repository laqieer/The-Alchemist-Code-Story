...
namespace SRPG
{
  ...
  public class FlowNode_PollSystem : FlowNodePersistent
  {
    ...
    private void Update()
    {
      if (!this.mStart)
        return;
      if (this.mCheckFlag[0] && !((Behaviour) EventSystem.current.currentInputModule).enabled)
        this.AnyOn();
      else if (this.mCheckFlag[1] && FadeController.InstanceExists && FadeController.Instance.IsFading())
        this.AnyOn();
