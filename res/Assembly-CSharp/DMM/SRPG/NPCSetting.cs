    ...
    public NPCSetting(JSON_MapEnemyUnit json)
    {
        ...
        for (int index1 = 0; index1 < json.abils.Length; ++index1)
        {
          ...
          if (json.abils[index1].skills != null)
          {
            ...
            for (int index2 = 0; index2 < json.abils[index1].skills.Length; ++index2)
            {
              ...
              this.abilities[index1].skills[index2].check_cnt = (OInt) json.abils[index1].skills[index2].check_cnt;
      ...
      if (json.acttbl != null && json.acttbl.actions != null && json.acttbl.actions.Length > 0)
      {
        ...
        for (int index = 0; index < json.acttbl.actions.Length; ++index)
        {
          ...
          aiAction.noExecAct = (eAIActionNoExecAct) json.acttbl.actions[index].noExecAct;
          aiAction.nextActIdx = json.acttbl.actions[index].nextActIdx;
          aiAction.nextTurnAct = (eAIActionNextTurnAct) json.acttbl.actions[index].nextTurnAct;
          aiAction.turnActIdx = json.acttbl.actions[index].turnActIdx;
      ...
      this.is_raid_boss = (OBool) (json.is_raid_boss == 1);
      this.need_dead = (OBool) (json.need_dead == 1);
      this.withdraw_drop = json.withdraw_drop != 0;
    }
  }
}
