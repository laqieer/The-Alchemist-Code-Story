...
namespace SRPG
{
  ...
  public class MailWindow : MonoBehaviour, IFlowInterface
  {
    ...
    private IEnumerator CoroutineActivateOutputLinks(int pinID)
    {
      ...
      return (IEnumerator) new MailWindow.\u003CCoroutineActivateOutputLinks\u003Ec__Iterator0()
      {
        pinID = pinID,
        \u0024this = this
      };
    ...
    private void UpdateUI()
    {
      ...
      if (UnityEngine.Object.op_Inequality((UnityEngine.Object) this.scrollRect, (UnityEngine.Object) null))
        this.scrollRect.SetNormalizedPosition(Vector2.one, true);
      ((Text) this.currentPageText).text = num.ToString();
      ((Text) this.maxPageText).text = pageMax.ToString();
      ((Selectable) this.nextPageButton).interactable = this.currentTabData.HasNext();
      ((Selectable) this.prevPageButton).interactable = this.currentTabData.HasPrev();
      ...
      {
        int count = this.currentTabData.currentPageData.mails.FindAll((Predicate<MailData>) (mailData => mailData.Contains(GiftTypes.IgnoreReceiveAll))).Count;
    ...
    private MailWindow.TabData currentTabData => this.GetTab(this.currentTab);

    private bool NexePage() => this.AddPage(1);

    private bool PrevPage() => this.AddPage(-1);
    ...
    internal class TabData
    {
      ...
      internal bool currentPageIsReady => this.currentPageData != null;

      internal MailPageData currentPageData => this.GetPageData(this.currentPage);
      ...
      internal bool HasPrev()
      {
        ...
        return pageData != null && pageData.hasPrev;
      }
