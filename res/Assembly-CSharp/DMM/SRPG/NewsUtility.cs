...
namespace SRPG
{
  ...
  {
    public static bool isNewsDisplay() => NewsUtility.getNewsTypes() != NewsUtility.NewsTypes.None;

    public static void clearNewsType() => GlobalVars.UrgencyPubHash = string.Empty;
    ...
    public static void setNewsState(string pub_hash, string urgency_pub_hash, bool force_display)
    {
      ...
      {
        MonoSingleton<UserInfoManager>.Instance.SetValue("PubHash", (object) pub_hash);
      ...
      if (string.IsNullOrEmpty(urgency_pub_hash) || !(str2 != urgency_pub_hash))
        return;
      MonoSingleton<UserInfoManager>.Instance.SetValue("UrgencyPubHash", (object) urgency_pub_hash);
