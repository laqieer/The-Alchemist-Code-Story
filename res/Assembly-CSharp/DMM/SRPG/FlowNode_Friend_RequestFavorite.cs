      ...
      else
      {
        ...
        else
        {
          ...
          if (friendData != null)
          {
            ...
            {
              UIUtility.SystemMessage((string) null, LocalizedText.Get("sys.FRIEND_ALREADY_FAVORITE"), (UIUtility.DialogResultEvent) (go => { }));
              return;
            }
          }
          this.mTargetFuid = fuid;
          this.ExecRequest((WebAPI) new ReqFriendFavoriteAdd(fuid, new Network.ResponseCallback(((FlowNode_Network) this).ResponseCallback)));
