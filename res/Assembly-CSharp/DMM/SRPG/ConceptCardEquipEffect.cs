...
namespace SRPG
{
  ...
  public class ConceptCardEquipEffect
  {
    ...
    public string ConditionsIname => this.mConditionsIname;

    public string Skin => this.mSkin;

    public SkillData CardSkill => this.mCardSkill;
    ...
    public AbilityData Ability
    {
      get
      {
        return this.is_levelmax && this.mAbilityLvMax != null ? this.mAbilityLvMax : this.mAbilityDefault;
      }
    }

    public BuffEffect AddCardSkillBuffEffectAwake => this.mAddCardSkillBuffEffectAwake;

    public BuffEffect AddCardSkillBuffEffectLvMax => this.mAddCardSkillBuffEffectLvMax;

    public bool IsExistAbilityLvMax
    {
      get
      {
        return this.mEffectParam != null && !string.IsNullOrEmpty(this.mEffectParam.abil_iname) && !string.IsNullOrEmpty(this.mEffectParam.abil_iname_lvmax);
      }
    }

    public bool IsDecreaseEffectOnSub
    {
      get => this.mEffectParam != null && this.mEffectParam.is_decrease_eff;
    }
      ...
      int awake_count_cap)
    {
      ...
      if (!string.IsNullOrEmpty(param.card_skill))
      {
        ...
        this.mCardSkill.Setup(param.card_skill, lv, lvcap, decreaseInfo: new ConceptCardEffectDecreaseInfo(conceptCard, param.is_decrease_eff));
      }
      ...
      if (!string.IsNullOrEmpty(param.statusup_skill))
      {
        ...
        this.mEquipSkill.Setup(param.statusup_skill, lv, lvcap, decreaseInfo: new ConceptCardEffectDecreaseInfo(conceptCard, param.is_decrease_eff));
      }
      ...
      if (!string.IsNullOrEmpty(param.abil_iname) && MonoSingleton<GameManager>.Instance.GetAbilityParam(param.abil_iname) != null)
      {
        ...
        this.mAbilityDefault.Setup((UnitData) null, 0L, param.abil_iname, lv - 1, lvcap, new ConceptCardEffectDecreaseInfo(conceptCard, param.is_decrease_eff));
      ...
      this.mAbilityLvMax.Setup((UnitData) null, 0L, param.abil_iname_lvmax, lv - 1, lvcap, new ConceptCardEffectDecreaseInfo(conceptCard, param.is_decrease_eff));
    ...
    public string GetBufText(BuffEffect effect, BuffEffect.BuffTarget target)
    {
      ...
      switch (target.calcType)
      {
        case SkillParamCalcTypes.Add:
          if (flag)
          {
            empty = LocalizedText.Get("sys.CONCEPT_CARD_SKILL_DESCRIPTION_CALC_ADD_PLUS", (object) num.ToString());
            break;
          }
          empty = LocalizedText.Get("sys.CONCEPT_CARD_SKILL_DESCRIPTION_CALC_ADD_MINUS", (object) num.ToString());
          break;
        case SkillParamCalcTypes.Scale:
          if (flag)
          {
            empty = LocalizedText.Get("sys.CONCEPT_CARD_SKILL_DESCRIPTION_CALC_UP", (object) num.ToString());
            break;
          }
          empty = LocalizedText.Get("sys.CONCEPT_CARD_SKILL_DESCRIPTION_CALC_DOWN", (object) num.ToString());
          break;
      }
    ...
    {
      return this.mEffectParam == null ? (BuffEffect) null : this.mEffectParam.CreateAddCardSkillBuffEffectAwake(awake, awake_cap);
    }

    public BuffEffect CreateAddCardSkillBuffEffectLvMax(int lv, int lv_cap, int awake)
    {
      return this.mEffectParam == null ? (BuffEffect) null : this.mEffectParam.CreateAddCardSkillBuffEffectLvMax(lv, lv_cap, awake);
    }
