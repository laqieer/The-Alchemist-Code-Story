...
namespace Gsc.Network.Data
{
  public class EntityRepository
  {
    ...
    {
      T obj;
      return EntityRepository.repository.GetRepository<T>().PublicList.TryGetValue(key, out obj) ? obj : (T) null;
    }
    ...
    public static void Update(Gsc.DOM.IObject root)
    {
      ...
      if (root.TryGetValue("update_models", out obj1))
      {
        foreach (IMember member in (IEnumerable<IMember>) obj1.GetObject())
        {
          ...
          if ((object) type != null)
          {
            AssemblySupport.MethodInfo constructor1 = AssemblySupport.GetConstructor(type, typeof (Gsc.DOM.IObject));
            IValue obj2;
            ...
            {
              AssemblySupport.MethodInfo constructor2 = AssemblySupport.GetConstructor(type, typeof (Dictionary<string, object>));
      ...
      if (root.TryGetValue("remove_models", out obj1))
      {
        foreach (IMember member in (IEnumerable<IMember>) obj1.GetObject())
        {
          ...
          if ((object) type != null)
          {
            ...
            foreach (IValue obj4 in (IEnumerable<IValue>) member.Value.GetArray())
              repository.Remove(obj4.ToString());
          }
        }
      }
    ...
    private class SimpleRepository<T> : EntityRepository.ISimpleRepository where T : Entity<T>
    {
      ...
      {
        object sender;
        if (this.entityList.TryGetValue(value.pk, out T _))
        {
