  ...
  public class GooglePlay : FlowWithPurchaseKit
  {
    ...
    protected override IWebTask CreateFulfillmentTask(PurchaseKit.PurchaseResponse response)
    {
      ...
      foreach (PurchaseKit.PurchaseData purchaseData in response.Values)
      {
        ...
        WebTask<Verify, Verify.Response> webTask = new Verify(productInfo == null ? (string) null : productInfo.CurrencyCode, productInfo == null ? 0.0f : productInfo.Price, purchase.Data1, purchase.Data0).ToWebTask(WebTaskAttribute.Reliable | WebTaskAttribute.Silent | WebTaskAttribute.Parallel);
