...
public class PhotonStreamQueue
{
  ...
  public PhotonStreamQueue(int sampleRate) => this.m_SampleRate = sampleRate;

  private void BeginWritePackage()
  {
    if ((double) Time.realtimeSinceStartup < (double) this.m_LastSampleTime + 1.0 / (double) this.m_SampleRate)
    {
  ...
  public bool HasQueuedObjects() => this.m_NextObjectIndex != -1;
